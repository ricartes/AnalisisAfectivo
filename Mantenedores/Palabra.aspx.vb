...
Partial Class Mantenedores_Palabra
    ...
    Public Function CargarGrilla() As String
        ...
        Try
            Dim vLista As List(Of CL_Palabra) = DT_Palabra.SelectPalabra(ddl_sentimiento.SelectedValue)
    ...
    Protected Sub Grilla_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles Grilla.RowUpdating
        Try
            ...
            Dim registro As New CL_Palabra
            registro.CODIGO = CType(fila.FindControl("tx_e_cod_palabra"), Label).Text
            registro.INTENSIDAD = CType(fila.FindControl("tx_e_intensidad"), TextBox).Text
            registro.CODIGO_SENTIMIENTO = CType(fila.FindControl("tx_e_sentimiento"), Label).Text

            DT_Palabra.PalabraUpdate(registro, Session("Usuario"))
            ScriptManager.RegisterStartupScript(Me, Page.GetType, "Script", "alert('Palabra editada correctamente');", True)
            Grilla.EditIndex = -1
            CargarGrilla()
    ...
    Protected Sub Grilla_RowDeleting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewDeleteEventArgs) Handles Grilla.RowDeleting
        Try
            Dim fila As GridViewRow = Grilla.Rows(e.RowIndex)
            Dim registro As New CL_Palabra
            registro.CODIGO = DirectCast(fila.FindControl("lb_cod_palabra"), Label).Text
            registro.CODIGO_SENTIMIENTO = DirectCast(fila.FindControl("lb_sentimiento"), Label).Text

            DT_Palabra.PalabraDelete(registro, Session("Usuario"))
